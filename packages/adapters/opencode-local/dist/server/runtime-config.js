...
function parseConfiguredModelRef(raw) {
...
export async function prepareOpenCodeRuntimeConfig(input) {
    ...
    if (configuredModel) {
        const providerEntry = isPlainObject(nextProvider[configuredModel.provider])
            ? { ...nextProvider[configuredModel.provider] }
            : {};
        ...
        if (!isPlainObject(providerModels[configuredModel.model])) {
            ...
            notes.push(`Registered configured model ${configuredModel.provider}/${configuredModel.model} in the runtime OpenCode config.`);
        }
    }
    const nextConfig = {
        ...existingConfig,
...
export function prepareManagedOpenCodeRemoteHomes(input) {
    if (!input.config.managedAiConnection)
        return;
    if (!input.runtimeRootDir)
        throw new Error("Managed OpenCode authentication requires an isolated remote runtime directory.");
