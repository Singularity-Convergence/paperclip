...
import { adapterExecutionTargetIsRemote, adapterExecutionTargetRemoteCwd, overrideAdapterExecutionTargetRemoteCwd, adapterExecutionTargetSessionIdentity, adapterExecutionTargetSessionMatches, adapterExecutionTargetUsesManagedHome, adapterExecutionTargetUsesPaperclipBridge, describeAdapterExecutionTarget, ensureAdapterExecutionTargetCommandResolvable, ensureAdapterExecutionTargetRuntimeCommandInstalled, prepareAdapterExecutionTargetRuntime, adapterExecutionTargetDuplexObservabilityRecorder, adapterExecutionTargetEnablesSandboxDuplexBridge, readAdapterExecutionTarget, readAdapterExecutionTargetHomeDir, resolveAdapterExecutionTargetTimeoutSec, resolveAdapterExecutionTargetCommandForLogs, runAdapterExecutionTargetProcess, runAdapterExecutionTargetShellCommand, startAdapterExecutionTargetPaperclipBridge, } from "@paperclipai/adapter-utils/execution-target";
import { asString, asNumber, asStringArray, parseObject, buildPaperclipEnv, buildRuntimeToolsEnv, joinPromptSections, buildInvocationEnvForLogs, ensureAbsoluteDirectory, ensurePaperclipSkillSymlink, ensurePathInEnv, refreshPaperclipWorkspaceEnvForExecution, renderTemplate, renderPaperclipWakePrompt, selectPaperclipTaskMarkdown, selectInitialCommunicationGuidance, isPaperclipRecoveryWakePayload, DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE, DEFAULT_PAPERCLIP_CONVERSATION_PROMPT_TEMPLATE, isPaperclipSkillSourceMissing, readPaperclipRuntimeSkillEntries, readPaperclipIssueWorkModeFromContext, resolveLegacyPaperclipDesiredSkillNames, } from "@paperclipai/adapter-utils/server-utils";
...
export async function ensureRemoteOpenCodeModelConfiguredAndAvailable(input) {
    ...
    if (probe.timedOut) {
        console.warn(`[opencode-local] Remote model availability probe for "${model}" timed out after ${probeTimeoutSec}s; proceeding with the configured model.`);
        return;
    }
    if ((probe.exitCode ?? 1) !== 0) {
        const detail = firstNonEmptyLine(probe.stderr) || firstNonEmptyLine(probe.stdout);
        console.warn(`[opencode-local] Remote \`opencode models\` could not run for "${model}"${detail ? ` (${detail})` : ""}; proceeding with the configured model.`);
        return;
    }
    const models = parseOpenCodeModelsOutput(probe.stdout);
    if (models.length === 0) {
        console.warn(`[opencode-local] Remote \`opencode models\` returned no models; proceeding with the configured model "${model}".`);
        return;
    }
...
async function ensureOpenCodeSkillsInjected(onLog, skillsEntries, desiredSkillNames, skillsHome = resolveOpenCodeSkillsHome({})) {
...
export async function execute(ctx) {
    ...
    if (!executionTargetIsRemote) {
        await ensureOpenCodeSkillsInjected(onLog, openCodeSkillEntries, desiredOpenCodeSkillNames, resolveOpenCodeSkillsHome(config));
    }
