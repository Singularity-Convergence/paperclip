...
export async function discoverOpenCodeModels(input = {}) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        const result = await runChildProcess(`opencode-models-${Date.now()}-${Math.random().toString(16).slice(2)}`, command, ["models", ...(input.refresh ? ["--refresh"] : [])], {
            cwd,
            env: runtimeEnv,
...
export async function ensureOpenCodeModelConfiguredAndAvailable(input) {
    ...
    try {
        ...
        console.warn(`[opencode-local] Model availability probe could not run for "${model}" (${err instanceof Error ? err.message : String(err)}); proceeding with the configured model.`);
        return [{ id: model, label: model }];
    }
    if (models.length === 0) {
        // The probe ran but returned nothing (e.g. a transient provider-auth blip).
        // Same reasoning as above: warn, don't block the run.
        console.warn(`[opencode-local] \`opencode models\` returned no models; proceeding with the configured model "${model}".`);
        return [{ id: model, label: model }];
    }
    if (!models.some((entry) => entry.id === model)) {
        ...
        try {
            ...
            console.warn(`[opencode-local] Model availability refresh failed for "${model}" (${err instanceof Error ? err.message : String(err)}); preserving the cached availability rejection.`);
        }
        const sample = models
            .slice(0, 12)
            .map((entry) => entry.id)
